...
impl GitxMcp {
    ...
    async fn issue_list(&self, Parameters(p): Parameters<IssueListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::issues::issue_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn issue_get(&self, Parameters(p): Parameters<IssueGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::issues::issue_get(&self.client, p, self.detected_repo.as_ref()).await)
    }

    #[tool(description = "Use this when you need to create a new issue in a repository. Provide a title and optionally a body, labels, milestone, and assignees. Labels and milestone require numeric IDs — use label_list and milestone_list to look them up first. Returns the created issue details. Fails with 404 if the repository is not found, or 403 if you lack permission.")]
    async fn issue_create(&self, Parameters(p): Parameters<IssueCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::issues::issue_create(&self.client, p, self.detected_repo.as_ref()).await)
    }

    #[tool(description = "Use this when you need to update an existing issue — change its title, body, state (open/closed), labels, assignees, or milestone. Labels and milestone require numeric IDs — use label_list and milestone_list to look them up first. Labels and assignees replace existing values (not additive). Returns the updated issue details.")]
    async fn issue_edit(&self, Parameters(p): Parameters<IssueEditParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::issues::issue_edit(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn issue_comment_list(&self, Parameters(p): Parameters<IssueCommentListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::issue_comments::issue_comment_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn issue_comment_create(&self, Parameters(p): Parameters<IssueCommentCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::issue_comments::issue_comment_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn pr_list(&self, Parameters(p): Parameters<PrListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pulls::pr_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn pr_get(&self, Parameters(p): Parameters<PrGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pulls::pr_get(&self.client, p, self.detected_repo.as_ref()).await)
    }

    #[tool(description = "Use this when you need to create a new pull request. Provide head branch (source), base branch (target), title, and optionally a body, labels, milestone, and assignees. Labels require numeric IDs — use label_list to look them up first. The head branch must exist and have commits ahead of base. Returns the created PR details. Fails with 404 if branches don't exist, or 409 if a PR already exists for these branches.")]
    async fn pr_create(&self, Parameters(p): Parameters<PrCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pulls::pr_create(&self.client, p, self.detected_repo.as_ref()).await)
    }

    #[tool(description = "Use this when you need to update a pull request — change its title, body, state (open/closed), labels, or assignees. Labels require numeric IDs from label_list. Labels and assignees replace existing values. Returns the updated PR details.")]
    async fn pr_edit(&self, Parameters(p): Parameters<PrEditParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pulls::pr_edit(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn pr_merge(&self, Parameters(p): Parameters<PrMergeParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pulls::pr_merge(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn pr_review_list(&self, Parameters(p): Parameters<PrReviewListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pull_reviews::pr_review_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn pr_review_create(&self, Parameters(p): Parameters<PrReviewCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pull_reviews::pr_review_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn pr_files(&self, Parameters(p): Parameters<PrFilesParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pull_files::pr_files(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn pr_diff(&self, Parameters(p): Parameters<PrDiffParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::pull_files::pr_diff(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn file_read(&self, Parameters(p): Parameters<FileReadParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::files::file_read(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn file_list(&self, Parameters(p): Parameters<FileListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::files::file_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn file_create(&self, Parameters(p): Parameters<FileCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::files::file_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn file_update(&self, Parameters(p): Parameters<FileUpdateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::files::file_update(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn file_delete(&self, Parameters(p): Parameters<FileDeleteParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::files::file_delete(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn tree_get(&self, Parameters(p): Parameters<TreeGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::files::tree_get(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn branch_list(&self, Parameters(p): Parameters<BranchListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::branches::branch_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn branch_create(&self, Parameters(p): Parameters<BranchCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::branches::branch_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn branch_delete(&self, Parameters(p): Parameters<BranchDeleteParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::branches::branch_delete(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn branch_protection_list(&self, Parameters(p): Parameters<BranchProtectionListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::branches::branch_protection_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn branch_protection_create(&self, Parameters(p): Parameters<BranchProtectionCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::branches::branch_protection_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn commit_list(&self, Parameters(p): Parameters<CommitListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::commits::commit_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn commit_get(&self, Parameters(p): Parameters<CommitGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::commits::commit_get(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn commit_diff(&self, Parameters(p): Parameters<CommitDiffParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::commits::commit_diff(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn commit_compare(&self, Parameters(p): Parameters<CommitCompareParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::commits::commit_compare(&self.client, p, self.detected_repo.as_ref()).await)
    }

    // ── Labels ──────────────────────────────────────────────────────

    #[tool(description = "Use this when you need to list all labels available in a repository. Returns label ID, name, color, and description for each label. Use the returned IDs when creating or editing issues and pull requests (issue_create, issue_edit, pr_create, pr_edit).")]
    async fn label_list(&self, Parameters(p): Parameters<LabelListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::labels::label_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn label_create(&self, Parameters(p): Parameters<LabelCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::labels::label_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn label_edit(&self, Parameters(p): Parameters<LabelEditParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::labels::label_edit(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn milestone_list(&self, Parameters(p): Parameters<MilestoneListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::milestones::milestone_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn milestone_get(&self, Parameters(p): Parameters<MilestoneGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::milestones::milestone_get(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn milestone_create(&self, Parameters(p): Parameters<MilestoneCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::milestones::milestone_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn release_list(&self, Parameters(p): Parameters<ReleaseListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::releases::release_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn release_get(&self, Parameters(p): Parameters<ReleaseGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::releases::release_get(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn release_create(&self, Parameters(p): Parameters<ReleaseCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::releases::release_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn repo_get(&self, Parameters(p): Parameters<RepoGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::repo::repo_get(&self.client, p, self.detected_repo.as_ref()).await)
    }

    #[tool(description = "Use this when you need to search for repositories by keyword across the Gitea instance. Returns full name, description, and star count for each matching repository.")]
    ...
    async fn tag_list(&self, Parameters(p): Parameters<TagListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::tags::tag_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn tag_create(&self, Parameters(p): Parameters<TagCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::tags::tag_create(&self.client, p, self.detected_repo.as_ref()).await)
    }

    // ── Wiki ────────────────────────────────────────────────────────

    #[tool(description = "Use this when you need to list all wiki pages in a repository. Returns title and slug for each page. Use the returned slug with wiki_get to read page content. Returns a message if the wiki is disabled for the repository.")]
    async fn wiki_list(&self, Parameters(p): Parameters<WikiListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::wiki::wiki_list(&self.client, p, self.detected_repo.as_ref()).await)
    }

    #[tool(description = "Use this when you need to read the content of a specific wiki page. Requires the page slug from wiki_list. Returns the page title and decoded markdown content.")]
    async fn wiki_get(&self, Parameters(p): Parameters<WikiGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::wiki::wiki_get(&self.client, p, self.detected_repo.as_ref()).await)
    }

    #[tool(description = "Use this when you need to create a new wiki page with a title and markdown content. Content is plain text (base64-encoding is handled automatically). Returns the created page title. Fails with 403 if wiki is disabled for the repository.")]
    async fn wiki_create(&self, Parameters(p): Parameters<WikiCreateParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::wiki::wiki_create(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    #[tool(description = "Use this when you need to list CI/CD workflows (Actions) configured in a repository. Tries the Actions API first, then falls back to listing workflow files in .gitea/workflows or .github/workflows. Returns workflow file names.")]
    async fn actions_workflow_list(&self, Parameters(p): Parameters<ActionsWorkflowListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::actions::actions_workflow_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn actions_run_list(&self, Parameters(p): Parameters<ActionsRunListParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::actions::actions_run_list(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn actions_run_get(&self, Parameters(p): Parameters<ActionsRunGetParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::actions::actions_run_get(&self.client, p, self.detected_repo.as_ref()).await)
    }
    ...
    async fn actions_job_logs(&self, Parameters(p): Parameters<ActionsJobLogsParams>) -> Result<CallToolResult, ErrorData> {
        map_err(crate::tools::actions::actions_job_logs(&self.client, p, self.detected_repo.as_ref()).await)
    }
}
...
impl ServerHandler for GitxMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some(
                "Gitea/Forgejo MCP server with 43 tools covering issues, PRs, files, branches, \
